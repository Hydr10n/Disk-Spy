				...
				case RegistryOperation::SetValue: {
					if (lpCommandLine == nullptr)
						...
						if (str == nullptr)
							...
							status = RegSetValueExW(hKey, lpName, 0, REG_SZ, reinterpret_cast<PBYTE>(str), sizeof(*str) * wnsprintfW(str, StrLen, LR"("%ls" %ls)", lpPath, lpCommandLine));
					}
				}	break;
